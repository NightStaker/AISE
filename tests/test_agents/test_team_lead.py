...
class TestTeamLeadAgent:
    ...
    def test_has_all_skills(self):
        ...
        expected = {"task_decomposition", "task_assignment", "conflict_resolution", "progress_tracking"}
        assert set(agent.skill_names) == expected

    def test_task_decomposition(self):
        agent, store = self._make_agent()
        artifact = agent.execute_skill("task_decomposition", {
    ...
    def test_task_assignment(self):
        agent, store = self._make_agent()
        artifact = agent.execute_skill("task_assignment", {
            "tasks": [
    ...
    def test_conflict_resolution(self):
        agent, store = self._make_agent()
        artifact = agent.execute_skill("conflict_resolution", {
            "conflicts": [{
